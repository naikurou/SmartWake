...
/**
 * Retourne le libellé de la période de la journée en fonction de l'heure.
 *
 * @param int|null $hour
 * @return string
 */
function getTimePeriodLabel(?int $hour = null): string {
    if ($hour === null) {
        $hour = (int)date('G');
    }
    if ($hour >= 6 && $hour < 9) {
 ...
 * @return array
 */
function getLuxLevelMeta(string $level, ?int $hour = null): array {
    if ($hour === null) {
        $hour = (int)date('G');
    }
    
    $label = getTimePeriodLabel($hour);
    
    $meta = [
        'NIGHT_FULL' => ['label' => $label, 'icon' => '⬛', 'css' => 'level-night-full',  'range' => '< 1 lux'],
        'NIGHT_DIM'  => ['label' => $label, 'icon' => '🌙', 'css' => 'level-night-dim',   'range' => '1–10 lux'],
        'DAWN'       => ['label' => $label, 'icon' => '🌅', 'css' => 'level-dawn',         'range' => '10–50 lux'],
        'MORNING'    => ['label' => $label, 'icon' => '🌤️', 'css' => 'level-morning',      'range' => '50–200 lux'],
        'DAY'        => ['label' => $label, 'icon' => '☀️', 'css' => 'level-day',          'range' => '200–500 lux'],
        'ALERT'      => ['label' => $label, 'icon' => '💡', 'css' => 'level-alert',        'range' => '> 500 lux'],
    ];
 ...
 * @return string
 */
function getDayStatus(int $lux, ?int $hour = null): string {
    if ($hour === null) {
        $hour = (int)date('G');
    }
    
    // En journée (de 7h à 21h)
    if ($hour >= 7 && $hour < 21) {
        return $lux >= 15 ? 'DAY' : 'NIGHT';
    }
    
    // En soirée / nuit (de 21h à 7h)
    return $lux >= 150 ? 'DAY' : 'NIGHT';
}
 ...
 * @return array
 */
function getWakeRecommendation(int $lux, string $status = '', ?int $hour = null): array {
    if ($hour === null) {
        $hour = (int)date('G');
    }
    ...
    // Si on est en pleine nuit (entre 22h et 6h)
    if ($hour >= 22 || $hour < 6) {
        if ($level === 'ALERT') {
            return [
                'optimal' => false,
                ...
                'detail'  => "Changement brusque ({$lux} lux) en pleine nuit. Quelqu'un a allumé la lumière.",
                'action'  => 'alert',
            ];
        }
        ...
        return [
            'optimal' => false,
            'level'   => $level,
            'message' => 'Nuit — Sommeil recommandé',
            'detail'  => "Il est {$hour}h ({$lux} lux). Mode veille nocturne actif.",
            'action'  => 'sleep',
        ];
    }
    
    // En dehors de la nuit (de 6h à 22h)
    switch ($level) {
        case 'NIGHT_FULL':
            return [
                'optimal' => false,
                ...
                'detail'  => "Il fait très sombre ({$lux} lux). Le réveil attend une lueur d'aube.",
                'action'  => 'sleep',
            ];
        case 'NIGHT_DIM':
            return [
                ...
                'detail'  => "Faible luminosité ({$lux} lux). Simulation d'aube douce.",
        ...
        case 'DAWN':
            ...
            return [
                'optimal' => $isMorning,
                'level'   => $level,
                'message' => $isMorning ? 'Aube naissante — Alarme douce' : 'Lumière d\'aube en journée',
                'detail'  => $isMorning 
                    ? "Lumière du matin ({$lux} lux). Moment idéal pour une alarme douce !"
                    : "Lumière faible de journée ({$lux} lux). Pas d'alarme requise.",
                'action'  => $isMorning ? 'soft_alarm' : 'day_mode',
            ];
        case 'MORNING':
            ...
            return [
                'optimal' => $isMorning,
                'level'   => $level,
                'message' => $isMorning ? 'Matin clair — Alarme principale' : 'Luminosité de journée',
                'detail'  => $isMorning
                    ? "Pièce bien éclairée ({$lux} lux). L'alarme principale se déclenche."
                    : "Luminosité modérée ({$lux} lux).",
                'action'  => $isMorning ? 'main_alarm' : 'day_mode',
            ];
        case 'DAY':
            return [
                'optimal' => false,
                'level'   => $level,
                'message' => 'Plein jour — Mode actif',
                'detail'  => "Lumière naturelle franche ({$lux} lux). L'écran est en mode actif.",
                'action'  => 'day_mode',
            ];
        case 'ALERT':
        default:
            return [
                'optimal' => false,
                'level'   => $level,
                'message' => '⚠️ Luminosité élevée ou soudaine',
                'detail'  => "Forte lumière détectée ({$lux} lux).",
                'action'  => 'alert',
            ];
    }
}
